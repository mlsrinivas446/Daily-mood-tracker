...
const addedDayToMonthsList = monthsList.map(eachMonth => ({
  ...eachMonth,
  dates: eachMonth.dates.map(each => ({
    ...each,
    day: '',
    emoji: '',
  })),
...
class App extends Component {
  state = {
    initialMonth: 1,
    initialMonthsList: [...addedDayToMonthsList],
    initialDaysList: [...daysList],
    initialEmojisList: [...emojisList],
    selectedDay: 'Sun',
    activeEmojiId: emojisList[0].id,
    selectedEmoji: 'Very Happy',
    emojiCount: 0,
    selectedDate: null,
    activeDayId: null,
  }
  ...
  handlePreviousMonth = () => {
    this.setState(prevState => ({
      initialMonth:
        prevState.initialMonth > 1 ? prevState.initialMonth - 1 : 12,
    }))
  }
  ...
  handleNextMonth = () => {
    this.setState(prevState => ({
      initialMonth:
        prevState.initialMonth < 12 ? prevState.initialMonth + 1 : 1,
    }))
  }

  calculateEmojiCount = (emoji, day, countmonthsList) => {
    const {initialMonth} = this.state
    const showPresentMonth = countmonthsList.find(
      each => each.month === initialMonth,
    )
    return showPresentMonth
      ? showPresentMonth.dates.filter(
          eachDate => eachDate.emojiName === emoji && eachDate.day === day,
        ).length
      : 0
  }
  ...
  setWeekDay = date => {
    ...
    if ([1, 8, 15, 22, 29].includes(parsedDate)) {
      day = 'Sun'
    } else if ([2, 9, 16, 23, 30].includes(parsedDate)) {
      day = 'Mon'
    } else if ([3, 10, 17, 24, 31].includes(parsedDate)) {
      day = 'Tue'
    } else if ([4, 11, 18, 25].includes(parsedDate)) {
      day = 'Wed'
    } else if ([5, 12, 19, 26].includes(parsedDate)) {
      day = 'Thu'
    } else if ([6, 13, 20, 27].includes(parsedDate)) {
      day = 'Fri'
    } else if ([7, 14, 21, 28].includes(parsedDate)) {
      day = 'Sat'
    }
    return day
  }
  ...
  updateMonthsList = ({
    ...
    if (!showPresentMonth)
      return {
        updatedMonthsList: initialMonthsList,
        emojiCount: 0,
        showPresentMonth: null,
      }

    const setPresentDate =
      showPresentMonth.dates.find(each => each.id === dateId) || {}

    const matchedactiveEmojiId =
      emojisList.find(eachEmoji => eachEmoji.id === activeEmojiId) || {}

    const updatedMonthsList = initialMonthsList.map(eachMonth => {
      if (eachMonth.month === initialMonth) {
        return {
          ...eachMonth,
          dates: eachMonth.dates.map(eachDate => {
            if (eachDate.id === dateId) {
              return {
                ...eachDate,
                emojiUrl:
                  matchedactiveEmojiId.emojiUrl !== eachDate.emojiUrl
                    ? matchedactiveEmojiId.emojiUrl
                    : '',
                emojiName:
                  matchedactiveEmojiId.emojiName !== eachDate.emojiName
                    ? matchedactiveEmojiId.emojiName
                    : '',
                day: this.setWeekDay(setPresentDate.date),
                emoji: selectedEmoji,
              }
            }
            return eachDate
    ...
    const emojiCount = this.calculateEmojiCount(
      setPresentDate.emoji || selectedEmoji,
      setPresentDate.day || selectedDay,
      updatedMonthsList,
    )
  ...
  setDate = id => {
    this.setState(prevState => {
      const {initialMonthsList} = prevState
      const {initialMonth, activeEmojiId, selectedDay, selectedEmoji} =
        this.state

      const {updatedMonthsList, emojiCount, setPresentDate} =
      ...
      return {
        selectedDate: setPresentDate.date || '',
        initialMonthsList: updatedMonthsList,
        emojiCount,
      }
    })
  }
  ...
  setEmoji = emoji => {
    this.setState(prevState => {
      const {initialMonthsList, selectedDay} = prevState
  ...
  setDay = day => {
    this.setState(prevState => {
      const {initialMonthsList, selectedEmoji} = prevState
      const emojiCount = this.calculateEmojiCount(
        selectedEmoji,
        day,
        initialMonthsList,
    ...
    return (
      <ReactContext.Provider
        value={{
          ...
          setactiveEmojiId: this.setactiveEmojiId,
          activeDayId,
        }}
      >
        <Switch>
